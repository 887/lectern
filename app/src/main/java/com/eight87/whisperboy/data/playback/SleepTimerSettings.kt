...
/**
 * DataStore-backed [SleepTimerSettings]. Mirrors [AndroidPlaybackSettings]:
 * `<typed>PreferencesKey` + `data.map { ... }` Flows + `edit { ... }` setters, with
...
class AndroidSleepTimerSettings(
    ...
    override val shakeToResume: Flow<Boolean> =
        dataStore.data.map { prefs -> prefs[KEY_SHAKE_TO_RESUME] ?: DEFAULT_SHAKE_TO_RESUME }
    ...
    override suspend fun setShakeToResume(enabled: Boolean) {
        dataStore.edit { it[KEY_SHAKE_TO_RESUME] = enabled }
    }

...
## Status: 🟡 PARTIAL — 5 rules open (B.1, C.2, E.1, E.2, G.1)
...
- [ ] **B.1** **Use `collectAsStateWithLifecycle`, not `collectAsState`.** The `WithLifecycle` variant defers Flow collection to `Lifecycle.STARTED`, which means DataStore reads (and the recomposition cascades their first emissions trigger) don't happen during the first composition pass. — _open: `app/src/main/java/com/eight87/whisperboy/ui/coverart/SelectCoverFromInternet.kt:96` uses `.collectAsState(initial = null)` instead of `collectAsStateWithLifecycle`; 57 other call sites in `ui/` already use the `WithLifecycle` variant correctly._
...
- [ ] **C.2** Prefer `Modifier.graphicsLayer { alpha = ... }` over `Modifier.alpha(...)` when alpha is derived from animated state. Same reason — defers the State read to draw, skips the composition entirely. — _open: `app/src/main/java/com/eight87/whisperboy/ui/playback/NowPlayingSheet.kt:160` (`.alpha(fullAlpha)`) and `:180` (`.alpha(miniAlpha)`) both read `sheetProgress`-derived state at composition; should use `Modifier.graphicsLayer { alpha = ... }`. `NowPlayingBar.kt:238` already uses the correct `graphicsLayer { scaleX = ... }` lambda form per its own inline comment._
...
- [ ] **E.1** Every `items(...)` / `items(count = ...)` call passes a `key = { ... }` argument that returns a stable identity (typically the entity id, NOT the index). **Duplicate-key crash gotcha** (tonearmboy `d75b542`): when a single `LazyVerticalGrid` mixes heterogeneous item types (e.g. section headers + book tiles), the section header's key MUST include a type discriminator (`"section:$letter"` not just `letter`) — otherwise it can collide with a tile id that happens to share the same value, and `LazyVerticalGrid` throws `IllegalArgumentException: Key X was already used`. Same rule applies when sort-aware section keys are introduced (per main.md E.3): keys must round-trip across sort changes without collision. — _open: `app/src/main/java/com/eight87/whisperboy/ui/playback/SleepTimerSheet.kt:110` calls `items(QUICK_PICKS)` without a `key =` argument (the chip values are stable `Int` minutes so it's low-risk, but the rule wants EVERY `items(...)` keyed); all 5 other `items(...)` call sites in `ui/` pass explicit stable keys._
- [ ] **E.2** When using `LazyColumn` / `LazyVerticalGrid` for a heterogeneous content type, consider `contentType = { ... }` too — Compose's slot table reuse benefits. — _open: grep for `contentType` in `ui/` returned 0 matches; `BookmarkScreen.kt` (stickyHeader + items) and `PlaybackScreen.kt` chapter queue are the candidates that would benefit. Advisory; not a violation per se but no surfaces have adopted it yet._

...
- [x] **R.F.8** `rememberSettingPicker<T>(...)` helper so each settings sub-page body is a `bindings` list, not 100+ LOC of `var xPicker by remember { mutableStateOf(false) }`. (Tonearmboy R.F.17 analog.) — _n/a for whisperboy: helper was prototyped in `ui/settings/SettingPicker.kt` and **retired as YAGNI** after the adoption pass found no natural consumer. Tonearmboy's settings catalog has one parent page with rows that open `AlertDialog` radio pickers (the shape this helper was built for). Whisperboy's K.2–K.5 chose a different shape: each knob group is its own routed sub-page (`PlaybackSettingsScreen`, `SleepTimerSettingsScreen`, `ThemeSettingsScreen`, `LibrarySortDefaultScreen`, `LibraryGridModeDefaultScreen`) with inline radios as the page body, not dialog content. No `var xPicker by remember { mutableStateOf(false) }` toggles exist anywhere in `ui/settings/`. Re-introduce only if whisperboy ever grows a multi-knob page with row-opens-dialog pickers._
- [x] **R.F.9** Detail-screen Flows on the data layer: `BookSource.observeBooksByAuthor(authorName)` etc. — UI does not filter in Compose. (Tonearmboy R.F.12 analog.) — _shipped: `BookDao.observeByAuthor(:authorName)` query (`WHERE author = :authorName COLLATE NOCASE`), `BookSource.observeBooksByAuthor` interface method + `LibraryRepository` impl, new `AuthorDetailRoute(authorName)` + `ui/library/AuthorDetailScreen.kt` rendering the pre-filtered Flow as a cover grid, entry point is the long-press action sheet's "View all by <author>" row. UI never filters — the Flow arrives ready-to-render. **shipped in commit `bc3bec5`**._

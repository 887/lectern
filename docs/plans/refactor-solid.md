...
=======
## Status: 🟡 PARTIAL — 3 rules still open after R.E ship. R.A complete (5/5). R.B partial — facets + no SettingsSnapshot shipped, but `Setting<T>` value type was not adopted; hand-rolled `*PreferencesKey` quartet used instead. R.C complete (6/6). R.D partial — `LibrarySorting.kt` shipped, but `PlaybackScreen.kt` (943 LOC) and `LibraryScreen.kt` (931 LOC) exceed the 500-LOC threshold; `BookCover` / `BookRow` / `LibraryTopBar` remain inline in `LibraryScreen.kt`. **R.E complete (6/6) — shipped in commit `5513541`: `WhisperboyApp.kt` shrunk 317 → 122 LOC, `RouteScope` + five per-grouping `routes/*Destinations.kt` files now hold every `entry<...>` block.** R.F partial — F.1/2/3/4/5/6/7/10 shipped, F.8/9 remain (no `rememberSettingPicker<T>`; no per-author detail flows yet).
>>>>>>> worktree-agent-a665eb26d4e54fa5f

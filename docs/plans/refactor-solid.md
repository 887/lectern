...
## Status: 🟡 PARTIAL — 7 rules still open after Phase P audit. R.A complete (5/5). R.B partial — facets + no SettingsSnapshot shipped, but `Setting<T>` value type was not adopted; hand-rolled `*PreferencesKey` quartet used instead. R.C complete (6/6). R.D partial — `LibrarySorting.kt` shipped, but `PlaybackScreen.kt` (943 LOC) and `LibraryScreen.kt` (931 LOC) exceed the 500-LOC threshold; `BookCover` / `BookRow` / `LibraryTopBar` remain inline in `LibraryScreen.kt`. R.E entirely open — `WhisperboyApp.kt` (317 LOC) has 16+ inline `entry<...>` blocks, no `RouteScope`. R.F partial — F.1/2/3/4/5/6/7/10 shipped, F.8/9 remain (no `rememberSettingPicker<T>`; no per-author detail flows yet).
...
- [ ] **R.B.1** Phase K introduces `Setting<T>` + `EnumSetting<E>` value types in `data/settings/` (or wherever fits — Phase K decides the package). — _open: no `Setting<T>` value type was introduced; each facet uses hand-rolled `*PreferencesKey + Flow + setter` (e.g. `data/playback/PlaybackSettings.kt:105-113`). Facets stay small enough that this hasn't bitten yet, but the rule as written is not satisfied._
- [ ] **R.B.2** Every Phase K key declared via `Setting<T>` — no hand-rolled `stringPreferencesKey + Flow + setter` quartet. — _open: see R.B.1; `intPreferencesKey` / `floatPreferencesKey` / `booleanPreferencesKey` quartet is used throughout `data/playback/`, `data/theme/`, `data/library/`._
...
- [ ] **R.D.1** No Compose UI file ships over 500 LOC. Soft heuristic: at 400 LOC, look for the next split. (CLAUDE.md SOLID-S already captures this.) — _open: `ui/playback/PlaybackScreen.kt` is 943 LOC and `ui/library/LibraryScreen.kt` is 931 LOC. Both exceed the threshold and house multiple composables that could split out._
...
- [ ] **R.D.4** `EmptyState`, `BookCover`, `BookRow`, `LibraryTopBar` get their own files when each crosses ~50 LOC of meaningful code. — _open: `LibraryEmptyState`, `BookGridTile` (BookCover analog), `BookListRow`, `SectionHeader`, `LibraryHealthBanner`, `BookActionSheet`, `LibrarySearchBar`, `ManageFoldersFolderTypeSheet` all live inline inside `LibraryScreen.kt` (931 LOC). They should split out._
...
- [ ] **R.E.2** When Phase F adds the player route: introduce `RouteScope` interface (`graph: AppGraph`, `backStack`, `snackbar`, `playback: TransportCommands`, the relevant settings facets, callbacks). — _open: no `RouteScope` interface exists. All routes are inline `entry<...>` blocks in `WhisperboyApp.kt:120-282` (16+ destinations)._
- [ ] **R.E.3** Sealed `Destination` interface; per-destination `Destination.Register(scope: RouteScope)` extensions. One file per destination grouping (`routes/PlayerRoute.kt`, `routes/BookmarkRoute.kt`, etc.). — _open: no `routes/` package; no `Destination.Register` extensions. Each `entry<XxxRoute>` is inline in `WhisperboyApp.kt`._
- [ ] **R.E.4** `WhisperboyApp.kt` stays under 150 LOC: theme + scaffold + a single `entryProvider { destination -> destination.Register(scope) }` block. — _open: `WhisperboyApp.kt` is 317 LOC with 16+ inline `entry<...>` blocks. Past the 150-LOC budget._
- [ ] **R.E.5** Cross-cutting controllers lifted into `remember*Controller(...)` helpers — sleep-timer-arm-from-deeplink, foreground-rescan, etc. — _open: `grep -rn 'remember.*Controller' app/src/main/java/com/eight87/whisperboy/` returns 0 matches; cross-cutting concerns live inline in `WhisperboyApp.kt`._
...
- [ ] **R.F.8** `rememberSettingPicker<T>(...)` helper so each settings sub-page body is a `bindings` list, not 100+ LOC of `var xPicker by remember { mutableStateOf(false) }`. (Tonearmboy R.F.17 analog.) — _open: no `rememberSettingPicker<T>` helper exists. Sub-pages roll their own picker dialogs (`TimePickerDialog` in `SleepTimerSettingsScreen.kt:432`, `FolderTypePickerSheet` in `LibrarySettingsScreen.kt:281`, etc.). Sub-pages aren't catastrophic (332-472 LOC) but the helper would compress them._

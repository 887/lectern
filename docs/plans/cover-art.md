...
- [x] **A.3** `CoverExtractor` interface; three impls + a dispatcher. *Shipped in this commit: narrow `CoverExtractor` `fun interface` in `data/library/parser/`; `Mp4CoverExtractor` reuses Phase I's `Mp4BoxParser` visitor to walk `moov.udta.meta.ilst.covr` and reads the nested `data` atom payload (skips 8-byte type-flags+reserved header); `MatroskaCoverExtractor` walks EBML `Segment\Attachments\AttachedFile` (id `0x61A7`, with `FileMimeType=0x4660` + `FileData=0x465C`) and returns the first `image/`-prefixed entry; `Mp3CoverExtractor` hand-rolls a minimal ID3v2.2 / v2.3 / v2.4 frame walker that returns the first `APIC` (or v2.2 `PIC`) image bytes. `CoverExtractorDispatcher` routes by MIME → file-extension → magic-byte sniff. `LibraryScannerEnrichment` is widened: when `Media3MediaAnalyzer` returns no embedded cover for any of the first 5 chapters, falls back to the dispatcher (dedupes by URI so SingleFile books only open their one source once). `AppGraph` instantiates the dispatcher and passes it to `LibraryScannerEnrichment`. `FileMetadata` was NOT widened with a `mimeType` field — the dispatcher does its own magic-byte sniff (mirroring `ChapterParser`'s detection), so the analyzer's MIME knowledge isn't load-bearing here. Unit tests for each extractor run against hand-rolled byte-array fixtures via the existing `ByteArraySeekableSource` (no Robolectric needed).*
...
- [x] **A.6** Per-book overflow action "Use custom cover from device" — `OPEN_DOCUMENT` for `image/*`, copy bytes via `CoverSaver`, mark with a flag (`coverSource = Custom`) so a later rescan doesn't overwrite the user's pick. *Shipped: `BookActionSheet` in `LibraryScreen.kt` gains the "Use custom cover from device" `TextButton`; `rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument())` filtered to `image/*`; bytes read via `contentResolver.openInputStream(uri).use { it.readBytes() }` and written through `BookSource.setCustomCover` → `CoverStore.writeCover` + `BookDao.setCoverPath` + `BookDao.setCoverSource`. New `CoverSource { Scanned, Custom }` enum + `CoverSourceConverter`, schema v2→v3 migration (`MIGRATION_2_3`), `BookEntity.coverSource` defaults to `Scanned`. `LibraryRepository.applyScan` fans out per-book to consult the existing row's `coverSource` and skips overwriting `Custom` covers.*

**Shipped:** A.1, A.2, A.4, A.5 in commit `05217dd`. A.6 shipped in commit `fa5f832`. A.3 shipped in this commit (cover-art A.3 — dedicated MP4 / Matroska / MP3 cover extractors).
